...
public class ContestService implements IContestService {
    ...
    private List<Question> pickQuestionsList(final List<Question> questions,final Integer numQuestion){
        List<Question> pickeQuestions = new ArrayList<>();
        List<Question> allQuestions = questions;
        Random random = new Random();
        for(int i=0;i<numQuestion;i++){
            int randomIndex = random.nextInt(allQuestions.size());
            pickeQuestions.add(allQuestions.get(randomIndex));
            allQuestions.remove(randomIndex);
        }
        return pickeQuestions;
    }
...
// Player player = playerRepository.findByName(name).orElseThrow(() -> new PlayerNotFoundException("Cannot Attend Contest. Contest for given id:"+name+" not found!"));
